...
### REQ-mm-quoter (NARROWED v2 — between-round only) — **Complete (2026-05-11, plan 04-05)**
...
- **Implementation (v2 / 04-05):** Pure-function `compute_half_spread(vega_between, staleness_s) -> int` + async `quote_mm_between_round(state, theo, market, mgr, ticker, count, *, now=None) -> None` at `src/quoting/mm_quoter.py` (~140 lines including module docstring). Formula: `base = max(MIN_HALF_SPREAD, ceil(MM_VEGA_SPREAD_K * sqrt(vega_between)))` + `penalty = max(0, floor(staleness_s - 2.0))` (1c/s above 2s; 5s pull-on-staleness is plan 04-03's kill_switch_staleness path). Ceiling on vega contribution defends against 3.01c-rounds-to-3c floor-tie that would silently violate Pitfall 4. `MM_VEGA_SPREAD_K=50.0` (TBD; tunes Phase 1's typical vega_between range [0.001, 0.01] into a [1.5c, 5c] vega-driven band on top of the 3c floor). Idempotent on unchanged-price + age < 2s; cancels stale (age > 2s) OR mispriced quotes BEFORE placing fresh; boundary guard skips placement when theo near 0.01/0.99 pushes prices outside [1, 99]. Every `Quote` tagged `strategy_id="MM_BETWEEN_ROUND"` for DEC-020 v2 separate-ledger routing — hypothetical fills land in `data/fills/{match_id}.mm_between_round.jsonl` via shared `maybe_record_mm_fill` helper from `src/quoting/fill_ledger.py` (co-owned by plans 04-05/04-06/04-07; routes by `quote.strategy_id`). Floor invariant verified hypothesis-style (200 examples over vega in [0, 1] x staleness in [0, 4.99]) AND explicitly via `test_spread_floor_beats_fee` (`MIN_HALF_SPREAD > ceil(0.035 * 0.5 * 0.5 * 100) / 100 + 1c slippage`). 15 GREEN tests in `tests/quoting/test_mm_quoter.py` + 14 GREEN tests in `tests/quoting/test_fill_ledger.py`.
